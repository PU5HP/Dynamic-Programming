...
public class Main {
    ...
    //TABULATION METHOD ITERATON TC-(O(n)) SC-O(n) slightly better than basic memoisationj with recursion method because of the less function call stack 
    ...
    public static int fib_tab_opt(int n) {
        //initialize the first two values
        int first= 0; int sec=1;int third=0;
        for(int i=2;i<=n;i++){
            third = first + sec;
            first = sec;
            sec =third;
